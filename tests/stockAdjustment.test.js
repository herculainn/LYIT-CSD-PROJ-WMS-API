...
const testEndpoint = 'http://localhost:' + process.env.PORT + '/api/stockadjustment/';
...
describe('API Test Endpoint: ./api/stockcount/', function() {
    ...
    beforeEach(async () => {
        ...
        seedStockItem.stockItemCounts = {
            create: { // create the relationship table instance
                ...
                stockItemCount: initialCount // required for the rel table
            }
        };
    ...
    describe('Using REQUEST BODY', function() {
        ...
        describe('POST', function () {
            ...
            it('Update stock count where relationship exists', async function () {
                ...
                assert.equal(postStockItemCount.stockItemCount, initialCount + addCount);
            });

            it('Update stock count where relationship exists using negative amount', async function () {

                const existingBinLocationID = (await prisma.binLocation.findMany({}))[0].id; // 'BinLocation WH001.A'
                const existingStockItemID = (await prisma.stockItem.findMany({}))[0].id; // 'NVIDIA'
                const remCount = initialCount / 2;
                ...
                assert.equal(postStockItemCount.stockItemCount, initialCount + remCount);
            });
            ...
            it('Set stock count where relationship does not exist but records do', async function () {
                ...
                assert.equal(postStockItemCount.stockItemCount, newAdjustment);
            });
    ...
    describe('Using PATH PARAMETER and QUERY STRING', function() {
        ...
        describe('POST', function () {
            ...
            it('Set stock count where relationship exists', async function () {
                ...
                assert.equal(postedStockItemCount.stockItemCount, initialCount + newCount);
            });

            it('Set stock count where relationship exists vwith negative amount', async function () {
